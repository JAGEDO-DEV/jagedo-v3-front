    ...
    const fetchProduct = async () => {
      if (isEditMode && productId) {
        if (isDraftMode) {
          ...
          const draft = savedDrafts.find((d: any) => d.id === productId);
          if (draft) {
  ...
  const handleApiSubmit = async (statusType: string) => {
    ...
    if (statusType === "Drafts") {
      try {
        ...
        if (isDraftMode) {
          const index = existingDrafts.findIndex((d: any) => d.id === productId);
          if (index > -1) {
        ...
        toast.success("Product saved to local drafts!");
        navigate(-1);
        return;
      } catch (error) {
    ...
    try {
      ...
      if (isEditMode && productId && !isDraftMode) {
