...
const ShopAppPage = ({ data: profileData }) => {
  ...
  const fetchSellerAndLocalProducts = async () => {
    ...
    try {
      ...
      const fetchedProducts = response.data || response.hashSet || response.items || [];
            ...
            {profileData ? (
              <>
                ...
                <div className="w-full flex gap-2 overflow-x-auto pb-2">
                  <button
                    type="button"
                    disabled={user?.userType !== "HARDWARE"}
                  ...
                  <button
                    type="button"
                    disabled={user?.userType !== "CONTRACTOR"}
                    className={`flex-1 min-w-[120px] rounded-lg border px-4 py-3 text-sm font-medium transition ${
