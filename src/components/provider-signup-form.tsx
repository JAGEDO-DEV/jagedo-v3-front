...
export function ProviderSignupForm({
  ...
  const findCloseDomain = (domain: string): string | null => {
    ...
    for (const known of KNOWN_DOMAINS) {
      ...
      // Only suggest if within edit distance of 2 (catches gtail→gmail, gnail→gmail etc.)
  ...
  const validateEmailFormat = (email: string) => {
    ...
    // Check hardcoded typo map first
    if (commonEmailTypos[domain]) {
      const suggestion = `${local}@${commonEmailTypos[domain]}`;
      return {
        valid: false,
    ...
    const closeDomain = findCloseDomain(domain);
    if (closeDomain) {
      const suggestion = `${local}@${closeDomain}`;
      return {
        valid: false,
        message: `Did you mean ${suggestion}?`,
        suggestion,
    ...
    // Known valid domain — allow
    if (KNOWN_DOMAINS.includes(domain)) {
      return { valid: true, message: "" };
    }

    // Plausible custom/business domain — allow
    ...
    return {
      valid: false,
      message:
        "Please enter a valid email domain (e.g. gmail.com, outlook.com)",
    };
  };
  ...
  useEffect(() => {
    ...
    if (!validation.valid) {
      setEmailStatus("idle"); // keeps button enabled so error shows on click
      return; // don't hit the API for invalid/typo emails
    }
    ...
    switch (currentStep) {
      case 1:
        if (providerType === "FUNDI" && !formData.skills) {
          ...
          (!formData.contractorTypes || formData.contractorTypes.trim() === "")
        ) {
          toast.error("Please select at least one contractor type");
          return false;
        } else if (
          providerType === "HARDWARE" &&
          (!formData.hardwareTypes || formData.hardwareTypes.trim() === "")
        ) {
      ...
      case 3:
        if (!formData.phone) {
          ...
          toast.error(
            "Please enter a valid phone number starting with 7 or 1 and 9 digits long",
          );
          return false;
        }
        break;
      ...
      case 1:
        if (providerType === "FUNDI") {
          ...
          const handleContractorTypeToggle = (value: string) => {
            const currentTypes = formData.contractorTypes
              ? formData.contractorTypes.split(",").filter(Boolean)
              : [];
            let updatedTypes;
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      const v = validateEmailFormat(val);
                ...
                {emailSuggestion ? (
                  <p className="text-amber-600 text-sm flex items-center gap-1">
                    ...
                    <button
                      ...
                      onClick={() => {
                        updateFormData({ email: emailSuggestion });
              ...
              <p className="text-gray-500">
                ...
                affiliates to the provided number.
              </p>
            </div>
          </div>
        );
            ...
            <div className="space-y-4">
              <div className="space-y-2 flex flex-col items-center">
                ...
                <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden shadow-sm transition-all duration-200 focus-within:border-[rgb(0,0,122)] focus-within:ring-2 focus-within:ring-[rgb(0,0,122)]/20 focus-within:shadow-md w-[70%] mx-auto">
                  <div className="px-3 py-3 bg-gray-50 text-gray-700 border-r border-gray-200 text-sm font-medium whitespace-nowrap">
                    🇰🇪 +254
                  </div>
                  <input
                    id="phone"
                ...
                <p className="text-xs text-gray-500 text-center">
                  Enter your 9-digit phone number starting with 7 or 1
                </p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                <RadioGroup
                  ...
                  onValueChange={(value) =>
                    updateFormData({ otpMethod: value })
                  }
                  className="flex flex-col items-center justify-center text-center"
                >
                  <label
                    htmlFor="email-otp"
                    className={`w-full max-w-xs sm:max-w-md border rounded-lg px-4 sm:px-6 md:px-10 py-3 cursor-pointer transition-all ${formData.otpMethod === "email" ? "border-[#00a63e] bg-[#00a63e]/5 shadow-md" : "border-gray-200 hover:border-gray-300"}`}
                  >
                    <RadioGroupItem
                      value="email"
                      id="email-otp"
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      >
                        <Mail
                          className={`h-5 w-5 sm:w-4 sm:h-4 ${formData.otpMethod === "email" ? "text-[#00a63e]" : "text-gray-500"}`}
                        />
                      </div>
                      <div>
                        <span
                  ...
                  <label
                    htmlFor="phone-otp"
                    className={`w-full max-w-xs sm:max-w-md border rounded-lg px-4 sm:px-6 md:px-10 py-3 cursor-pointer transition-all ${formData.otpMethod === "phone" ? "border-[#00a63e] bg-[#00a63e]/5 shadow-md" : "border-gray-200 hover:border-gray-300"}`}
                  >
                    <RadioGroupItem
                      value="phone"
                      id="phone-otp"
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      >
                        <Phone
                          className={`h-5 w-5 sm:w-4 sm:h-4 ${formData.otpMethod === "phone" ? "text-[#00a63e]" : "text-gray-500"}`}
                        />
                      </div>
                      <div>
                        <span
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="space-y-4">
              ...
              <p className="text-gray-500">
                ...
                {formData.otpMethod === "email" ? "email" : "phone"}
              </p>
              <div className="space-y-2">
                <div className="relative">
                  <Input
          ...
          <div className="space-y-6 animate-fade-in max-w-md mx-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="mt-2 space-y-1.5">
                <p
                  className={`text-sm flex items-center gap-2 transition-colors ${formData.password && formData.password.length >= 8 ? "text-green-600" : "text-red-500"}`}
                >
                  {formData.password && formData.password.length >= 8 ? (
                ...
                <p
                  className={`text-sm flex items-center gap-2 transition-colors ${formData.password && /[A-Z]/.test(formData.password) ? "text-green-600" : "text-red-500"}`}
                >
                ...
                <p
                  className={`text-sm flex items-center gap-2 transition-colors ${formData.password && /\d/.test(formData.password) ? "text-green-600" : "text-red-500"}`}
                >
                ...
                <p
                  className={`text-sm flex items-center gap-2 transition-colors ${formData.password && /[^A-Za-z0-9]/.test(formData.password) ? "text-green-600" : "text-red-500"}`}
                >
                  {formData.password &&
      ...
      <div className="flex justify-between pt-4">
        ...
        {currentStep !== 5 && (
          <Button
            ...
            disabled={
              isSubmitting ||
              (currentStep === 2 && emailStatus === "checking") ||
